...
var wizard = function(){
	...
	function setupStep1()
	{
		...
		<fieldset> \
				...
				<button type="button" class="pure-button pure-button-primary" onClick="$(\'#dbPath\').val(JakilidUI.browseSaveFile($(this).val()));">' + tr('Browse', 'wizard') + '</button> \
		</fieldset> \
	</form>' + 
		'</div>' +
		...
		<fieldset> \
				...
				<button type="button" class="pure-button pure-button-primary" onClick="$(\'#dbExistingPath\').val(JakilidUI.browseOpenFile($(this).val()));">' + tr('Browse', 'wizard') + '</button> \
		</fieldset> \
	</form>' + 
		'</div>' +
		'</div>' +
	...
	function processDatabaseStep()
	{
		...
		if (dbType === 0)
		{
			...
			if (validateDbFile(tmpProfile.db, true))
		...
		else if (dbType === 1)
		{
			...
			if (validateDbFile(tmpProfile.db, false))
	...
	function validateDbFile(db, isNew)
	{
		...
		var err = JakilidUI.isValidDbFile(db, isNew);
		if (err != '')
		{
