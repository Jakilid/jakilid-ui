...
(function(){
	...
	$(window).resize(function() {
		...
		accountList.height(h-180);
		findCount = Math.ceil(((h-180)/70)+3);	//always find 3 more accounts than how many account which could fit accountList height
	...
	function setupUI()
	{
		...
		if (typeof JakilidUI.isMaximized === 'function' && JakilidUI.isMaximized())
		{
	...
	function findMore()
	{
		...
		JakilidWrapper.find(findQuery, findCount, displayingAccountCount, addAccounList);
	}
	...
	*/
	...
	function updateAccCount()
	{
		...
		if (totalQueryResults <= 0)
			$('.ResultStat').text( findQuery && findQuery != '' ?
								   tr('Nothing matched "%1"', [findQuery])/*.arg(findQuery)*/ :
								   tr('There\'s no item to display. Try adding a new one.')
								 );
		else
		{
			var stat;
			if (findQuery && totalQueryResults>1)
				if (cnt < totalQueryResults)
					stat= tr('Displaying %1 out of %2 items containing "%3"', [locale.toLocalNumber(cnt), locale.toLocalNumber(totalQueryResults), findQuery])/*.arg(locale.toLocalNumber(cnt), locale.toLocalNumber(totalQueryResults), findQuery)*/;
				else
					stat= tr('Displaying %1 items containing "%2"', [locale.toLocalNumber(cnt), findQuery])/*.arg(locale.toLocalNumber(cnt), findQuery)*/;
			else if (findQuery && totalQueryResults==1)
				stat= tr('Displaying the only item containing "%1"', [findQuery])/*.arg(findQuery)*/;
			else if (totalQueryResults>1)
				if ( cnt< totalQueryResults)
					stat= tr('Displaying %1 out of %2 items', [locale.toLocalNumber(cnt), locale.toLocalNumber(totalQueryResults)])/*.arg(locale.toLocalNumber(cnt), locale.toLocalNumber(totalQueryResults))*/;
				else
					stat= tr('Displaying %1 items', [locale.toLocalNumber(cnt)])/*.arg(locale.toLocalNumber(cnt))*/;
			else if (totalQueryResults==1)
	...
	function updateSearchResults(accId)
	{
		...
		JakilidWrapper.find(findQuery, count, 0, function(resultsObj){
