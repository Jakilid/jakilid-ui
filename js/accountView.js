...
function AccountViewer()
{
	...
	this.addInfo = /*void*/ function(info, isMute /*optional boolean*/) 
	{
		...
		if (this.isEdit())
		{
			...
			xEditableTypeChange('#changeInfoType-' + infoUid);
			this.fixSortableBug($('.inlineInput')); //FIX: sortable input click bug
		}
		if (typeof isMute === 'undefined' || !isMute)	//if isMute is true do not set account's changed property
		{
			...
			this.setupAutoTitleFill();//automatic fill title for new account
		}
...
//		if (this.isEdit())
//			return true;	//already in edit mode, so caller has met requirments (return true)
		...
		xEditable('#account-description');
		
		xEditable('.infoValue');
		xEditable('.infoKey');
		//$('.infoValue').inLineEdit('input').attr('dir', 'auto');	//make value direction auto. example: url in rtl theme
		xEditableTypeChange('span[id^="changeInfoType-"]');	
		...
		this.fixSortableBug($('#account-info .inlineInput'));
		
		return true;
	}
	
	this.setupAutoTitleFill = function()
	{
		...
		if ($('#account').attr('data-id') != "-1" && $('#account-title').inLineEdit('value') != '' )
			return;
		$('#account tr:not([data-typeid=1])').first().find('td').last().find('.infoValue').inLineEdit('input')	//select first not password type value
		.add($('#account tr[data-typeid=4]').first().find('td').last().find('.infoValue').inLineEdit('input'))	//select first url type value
		.change(function()
		{
	...
	this.fixSortableBug = function(item)
	{
	...
	function xEditableTypeChange(selector)
	{
	...
	function changeInfoType(infoId, typeId)
	{
		...
		valueChanged = $('#v-' + infoId).hasClass("editable-unsaved");
		valueEmpty = $('#v-' + infoId).hasClass("editable-empty");
		$('#v-' + infoId).inLineEdit('destroy');	//remove editable
		$('#v-' + infoId).replaceWith( createValueSpan('TODO:', (valueEmpty ? '' : $('#v-' + infoId).text()), typeId, infoId) );
		xEditable('#v-' + infoId);	//make it editable
		if (valueChanged)
			$('#v-' + infoId).addClass("editable-unsaved");
		if (valueEmpty)
			$('#v-' + infoId).addClass("editable-empty");
			
		that.fixSortableBug( $('#v-' + infoId).inLineEdit('input') );
	}

	function createValueSpan(key, value, type, uid)
	{
		//NOTE: pre is ued for type of inputs instead of span because span displays multiple spaces aa single one!
		return $('<pre></pre>', {'class':'infoValue', 'data-type': infoTypeName(type), 'data-title': tr('Enter value','account'), id: 'v-' + uid, 'data-maxwidth': '200', text: value});
		//return '<pre class="infoValue" data-type="' + infoTypeName(type) + '" data-title="' + tr('Enter value','account') + '" id="v-' + uid + '" data-maxwidth="200">' + htmlEncode(value) + '</pre>';
	}
